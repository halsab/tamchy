...
const changedAudio = catalog.categories[0]!.items[0]!.labelAudio;
...
for (const failure of [false, true]) {
  test(`T15–T16: ${failure ? 'неудачная B сохраняет A, явный повтор' : 'A→B без прерывания игры'}, несколько окон и соседнее приложение`, async ({
    page,
    context,
    browserName,
  }, testInfo) => {
    test.setTimeout(60_000);
    const b = process.env.TAMCHY_UPDATE_DIST;
    expect(
      b,
    ...
    try {
      ...
      const hashA = createHash('sha256')
        .update(await readFile(resolve('dist', changedAudio)))
        .digest('hex');
      ...
      const prompt = await page.getByRole('main').innerText();
      const target = catalog.categories[0]!.items.find((item) =>
        prompt.includes(item.promptTt),
      )!;
      let navigations = 0;
      ...
      if (failure) {
        await expect(updateStatus(other)).toHaveText(strings.pwa.updateError, {
          timeout: 20_000,
        });
        await expect(status(other)).toHaveText(strings.status.offlineReady);
        await server.stop();
        if (browserName === 'chromium') await context.setOffline(true);
        expect(await audioHash(page)).toBe(hashA);
        await expect(status(other)).toHaveText(strings.status.offlineReady);
        await server.start();
        await context.setOffline(false);
        server.fail(() => false);
        // Повтор через экран взрослых, без принудительной активации.
        await other.getByRole('button', { name: strings.action.retry }).click();
      }
      ...
      await page.close();
      await expect(updateStatus(other)).toHaveText(strings.pwa.updateWaiting);
      expect(await audioHash(other)).toBe(hashA);
      ...
      await expect(
        reopened.getByText(`${strings.parents.about} Яңа версия.`, {
          exact: true,
        }),
      ).toBeVisible();
